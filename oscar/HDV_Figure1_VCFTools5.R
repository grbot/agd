...
delta = 0.4
popList <- c('Benin', 'Botswana', 'Burkina', 'Cameroon', 'Ghana', 'Mali', 'Nigeria', 'Zambia') # Here you need to include the names of all your populations.
...
for (pop1 in 1:(length(popList)-1)) {
  for (pop2 in (pop1+1):length(popList)) {
    ...
    if (HDVcounter > 0) { 
      write.table(out, paste(chr,"HDVList_",popList[pop1],"-",popList[pop2],"Delta40.txt",sep=""),quote=FALSE, col.names=FALSE, row.names=FALSE, sep="\t")
    ...
    write.table(table(frqList), paste(chr,popList[pop1],"-",popList[pop2], "_delta40Distribution.txt",sep=""), quote=FALSE, col.names=FALSE, row.names=FALSE, sep="\t")
