...
for (pop1 in 1:(length(popList)-1)) {
  for (pop2 in (pop1+1):length(popList)) {
    ...
    out <- c()
    for (chr in new:new) {
      pop1FrqFile <- read.delim2(paste(popList[pop1],frqFileExtension, sep=""), skip=1, header=F, sep=" ", stringsAsFactors=FALSE)
      pop2FrqFile <- read.delim2(paste(popList[pop2],frqFileExtension, sep=""), skip=1, header=F, sep=" ", stringsAsFactors=FALSE)
      head(pop1FrqFile)
    ...
    if (HDVcounter > 0) { 
      write.table(out, paste(chr,"HDVList_",popList[pop1],"-",popList[pop2],"Delta60.txt",sep=""),quote=FALSE, col.names=FALSE, row.names=FALSE, sep="\t")
    ...
    write.table(table(frqList), paste(chr,popList[pop1],"-",popList[pop2], "_delta60Distribution.txt",sep=""), quote=FALSE, col.names=FALSE, row.names=FALSE, sep="\t")
    
    pdf(file = paste(chr,"Figure1_",popList[pop1],popList[pop2],"_Delta", delta, "Distribution.pdf",sep=""),width = 16, height=7)
    ...
    HDV1 <- deltaVector[which(deltaVector <= -0.6)]
    HDV2 <- deltaVector[which(deltaVector >= 0.6)]
    if (HDVcounter > 0) {
      if ( HDVcounter < 10) {
        yax_max <- 10
      } else {
