...
do
  #runhdv.sh correct chromosome results are in the fifth column; copy these into the first to replace chr25
  awk '$1=$5' 25HDVList_${poppair[$k]}Delta60.txt | awk '{print $1"\t"$2"\t"$2"\t"$3"\t"$4"\t"$5}' > 25HDVList_${poppair[$k]}Delta60_2.txt   
  ...
  #HDV_bedtoolsintersect.sh  Intersect between the hdv file and 5kb windows keeping all the windows with HDVs
  bedtools intersect -wa  -wb -a chrall_5K.bed  -b 	25HDVList_${poppair[$k]}Delta60_2.txt	>	${arr[$k]}_HDV_5K.bed
...
for k in `seq 0 20`;
    do
      ...
      #after5kb_filt.sh count number of lines for all population pairs after 5kb filtering
        wc -l ${arr[$k]}_HDV_5K2.bed >> 5kb_filt.txt
    done
  ...
  rm -f genelist_${arr[$k]}.txt
    ...
    do
      ...
      do
        ...
        while IFS='|' read -ra ANN; 
        do
          declare -a ANN="${ANN[@]}"
          
          echo ${ANN[4]} >> genelist_${arr[$k]}.txt
