...
POPS.each {
	...
	listWithHidden = file("${in_dir}/${it}/*${params.alltype}.vcf.gz", hidden: false)
	listWithHidden.each {
		vcfs.add("$it")
   		println "File ${it}"
	}
	...
	listWithHidden = file("${in_dir_freq}/${it}/*.frq.count", hidden: false)
	listWithHidden.each {
		ffreqcounts.add("$it")
   		println "File ${it}"
	}
...
process statsRareSDTVariants {
	...
	"""
	python3 ${params.scripts}/python/RareVariantsSDTtons.py  --freq-input ${ffreqcount}  --output-folder ${params.output_dir}/VariantsFreq --out ${file(ffreqcount).getName()}
...
process combineStatsRareSDTVariants {
	...
	script:
		"""
		python3 ${params.scripts}/python/CombineFreqs.py  --freqs-folder ${params.output_dir}/VariantsFreq 
